		...
		switch(type){
			case MEDIA_PLAYER: 
				if(xSemaphoreTake( s_mutex_player, ( TickType_t ) 0 ) != pdTRUE){
					ESP_LOGE(TAG,"Only one obj of player(such as meidaPlyaer/audioTrack)  can run!");
					return JOSHVM_FAIL;
				}
				...
				if(m_player_obj_created_status != OBJ_CREATED){
					...
					joshvm_media->j_union.mediaPlayer.status = AUDIO_UNKNOW;
					*handle = joshvm_media;			
			...
			case MEDIA_RECORDER: 	
				...
				if(m_rec_obj_created_status != OBJ_CREATED){
					...
					joshvm_media->j_union.mediaRecorder.status = AUDIO_UNKNOW;
					*handle = joshvm_media;
			...
			case AUDIO_RECORDER:	
				...
				if(a_rec_obj_created_status != OBJ_CREATED){
					...
					joshvm_media->j_union.audioRecorder.status = AUDIO_UNKNOW;
					*handle = joshvm_media;
...
int joshvm_esp32_media_close(joshvm_media_t* handle)
{
	...
	switch(handle->media_type)
	{
		case MEDIA_PLAYER:
			...
			handle->j_union.mediaPlayer.status = AUDIO_CLOSE;
			ESP_LOGI(TAG,"MediaPlayer closed!");			
			break;
		case MEDIA_RECORDER:
			if(handle->j_union.mediaRecorder.status == AUDIO_START){
				ESP_LOGE(TAG,"Can't release MediaRecorder when recording,need to stop it!");
				return JOSHVM_FAIL;
			}			
			...
			handle->j_union.mediaRecorder.status = AUDIO_CLOSE;
		...
		case AUDIO_RECORDER:
			...
			handle->j_union.audioRecorder.status = AUDIO_CLOSE;
...
int joshvm_esp32_media_prepare(joshvm_media_t* handle, void(*callback)(void*, int))
{
	...
	switch(handle->media_type){
		case MEDIA_PLAYER:
			if(handle->j_union.mediaPlayer.status == AUDIO_UNKNOW){
				handle->j_union.mediaPlayer.status = AUDIO_PREPARE;
			...
			break;
		case MEDIA_RECORDER:	
			if(handle->j_union.mediaRecorder.status == AUDIO_UNKNOW){
				handle->j_union.mediaRecorder.status = AUDIO_PREPARE;
...
int joshvm_esp32_media_stop(joshvm_media_t* handle)
{
	...
	switch(handle->media_type){
		...
		case MEDIA_RECORDER:
			if(handle->j_union.mediaRecorder.status == AUDIO_START){
				handle->j_union.mediaRecorder.status = AUDIO_STOP;
...
int joshvm_esp32_media_reset(joshvm_media_t* handle)
{
	...
	switch(handle->media_type){
		case MEDIA_PLAYER:	
			if(handle->j_union.mediaPlayer.status == AUDIO_START){
				ESP_LOGE(TAG,"Can't reset MediaPlayer when playing!");
				return JOSHVM_FAIL;
			}
			handle->j_union.mediaPlayer.status = AUDIO_UNKNOW;
			ret = JOSHVM_OK;
			break;
		case MEDIA_RECORDER:
			if(handle->j_union.mediaRecorder.status == AUDIO_START){
				ESP_LOGE(TAG,"Can't reset MediaRecorder when recording!");
				return JOSHVM_FAIL;
			}			
			...
			handle->j_union.mediaRecorder.status = AUDIO_UNKNOW;
			ret = JOSHVM_OK;
			break;
		default :
			ret = JOSHVM_NOT_SUPPORTED;
...
int joshvm_esp32_media_set_audio_sample_rate(joshvm_media_t* handle, uint32_t value)
{
	...
	switch(handle->media_type){
		case MEDIA_PLAYER:					
			...
			break;
		case MEDIA_RECORDER:
			if((handle->j_union.mediaRecorder.status == AUDIO_UNKNOW) || (handle->j_union.mediaRecorder.status == AUDIO_STOP)){
		...
		case AUDIO_TRACK:
			if((handle->j_union.audioTrack.status == AUDIO_UNKNOW) || (handle->j_union.audioTrack.status == AUDIO_STOP)){
				...
				return JOSHVM_OK;
			}else if(handle->j_union.audioTrack.status == AUDIO_CLOSE){
				return JOSHVM_FAIL;
			}
			ESP_LOGW(TAG,"Please set sample rate before audioPlayer started!");
			ret = JOSHVM_OK;
			break;
		case AUDIO_RECORDER:
			if((handle->j_union.audioRecorder.status == AUDIO_UNKNOW) || (handle->j_union.audioRecorder.status == AUDIO_STOP)){
				handle->j_union.audioRecorder.sample_rate = value;
				return JOSHVM_OK;				
			}else if(handle->j_union.audioRecorder.status == AUDIO_CLOSE){
				return JOSHVM_FAIL;
			}
			ESP_LOGW(TAG,"Please set sample rate before audioRecorder started!");
			ret = JOSHVM_OK;
			break;
		default :
			ret = JOSHVM_NOT_SUPPORTED;
...
int joshvm_esp32_media_set_channel_config(joshvm_media_t* handle, uint8_t value)
{
	...
	switch(handle->media_type){
		...
		case AUDIO_TRACK:
			...
			ESP_LOGW(TAG,"Please set channel before audioPlayer started!");
			ret = JOSHVM_OK;
			break;
		case AUDIO_RECORDER:
			if((handle->j_union.audioRecorder.status == AUDIO_UNKNOW) || (handle->j_union.audioRecorder.status == AUDIO_STOP)){
			...
			ESP_LOGW(TAG,"Please set channel before audioRecorder started!");
			ret = JOSHVM_OK;			
			break;
		default :
			ret = JOSHVM_NOT_SUPPORTED;
...
{					
	...
	switch(handle->media_type){
		...
		case AUDIO_TRACK:
			...
			ESP_LOGW(TAG,"Please set bit rate before audioPlayer started!");
			ret = JOSHVM_OK;
			break;
		case AUDIO_RECORDER:
			if((handle->j_union.audioRecorder.status == AUDIO_UNKNOW) || (handle->j_union.audioRecorder.status == AUDIO_STOP)){
			...
			ESP_LOGW(TAG,"Please set bit rate before audioRecorder started!");
			ret = JOSHVM_OK;
			break;
		default :
			ret = JOSHVM_NOT_SUPPORTED;
...
int joshvm_esp32_media_set_source(joshvm_media_t* handle, char* source)
{
	...
	switch(handle->media_type){
		case MEDIA_PLAYER:
			...
			ESP_LOGW(TAG,"Please set source before mediaPlayer started");
			ret = JOSHVM_OK;
			break;
		default :
...
int joshvm_esp32_media_get_position(joshvm_media_t* handle, int* pos)
{
	...
	switch(handle->media_type){
		case MEDIA_PLAYER:
			if(joshvm_audio_position_get(pos) != ESP_OK){
				return JOSHVM_FAIL;
			}
			ret = JOSHVM_OK;
			break;
...
int joshvm_esp32_media_get_duration(joshvm_media_t* handle, int* duration)
{
	...
	switch(handle->media_type){
		case MEDIA_PLAYER:
			if(joshvm_audio_time_get(duration) != ESP_OK){
				return JOSHVM_FAIL;			
			}	
			ret = JOSHVM_OK;
			break;
